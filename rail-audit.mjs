...
/* Pull the balanced { ... } that follows the first comma of a publishToolResult call,
   then read the top-level identifier keys out of it. Brace-depth aware, so nested
   objects and ternaries do not leak their inner keys into the result. */
function publishKeys(src) {
  const keys = new Set();
  let i = 0;
  while ((i = src.indexOf("publishToolResult(", i)) !== -1) {
    i += "publishToolResult(".length;
    ...
    // top-level keys only: identifier followed by ':' at depth 1
    let d = 0, s = null;
