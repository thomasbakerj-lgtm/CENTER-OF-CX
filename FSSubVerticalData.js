...
export const fsSubVerticals = {
  "retail-banking": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.retail-banking.aht]]", note: "Driven by the mix of quick transactional calls and longer lending and dispute calls" },
      { metric: "FCR", avg: "[[fs.sv.retail-banking.fcr]]", note: "Authentication steps and lookups across several systems stand in the way of single-contact resolution" },
      { metric: "CSAT", avg: "[[fs.sv.retail-banking.csat]]", note: "Moves with authentication friction, hold time and whether the issue is resolved" },
      { metric: "Containment", avg: "[[fs.sv.retail-banking.containment]]", note: "Balance checks and card activations are the automatable share; disputes and fraud stay with people" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
        ...
        stack: [
          { name: "NICE Nexidia", role: "Interaction Analytics", why: "Speech and text analytics used in banking. Automated compliance scoring, sentiment detection, and root-cause analysis across large interaction volumes.", href: "/vendors/analytics" },
        ...
        pitfall: "Recording platforms and analytics platforms are separate procurement decisions. Banks often buy NICE for recording but Verint for analytics, creating data silos where compliance recordings can't be analyzed without ETL pipelines that add a day or more of latency. Choose a unified stack or budget for real-time integration."
      },
      { layer: 6, name: "Routing & Orchestration",
        ...
        stack: [
          { name: "Genesys Cloud", role: "Primary CCaaS", why: "Intent-based and predictive routing that uses AI to match customer intent with a suitable agent. Built for large, multi-line banking operations.", href: "/vendors/genesys" },
      ...
      { layer: 5, name: "Conversation Management",
        ...
        stack: [
          ...
          { name: "LivePerson", role: "Conversational AI + Messaging", why: "Conversational AI and messaging with a banking customer base. Async messaging lets customers continue conversations across sessions. Strong Apple Business Chat and WhatsApp integrations.", href: "/vendors/digital-engagement" },
        ],
      ...
      { layer: 4, name: "Reasoning & Planning",
        ...
        stack: [
          { name: "Kasisto (KAI)", role: "Banking AI Assistant", why: "Conversational AI built for banking. Pre-trained on banking intents (balance, transactions, payments, card management).", href: "/vendors/iva" },
          ...
          { name: "Google CCAI", role: "AI Platform", why: "Dialogflow CX provides the NLU layer; Agent Assist provides real-time guidance. Suits banks with Google Cloud infrastructure and engineering teams that want to build custom.", href: "/vendors/iva" },
        ],
        pitfall: "Banking bots need real-time core banking data to be useful. A bot that says 'I'll check your balance' and then responds with yesterday's batch data destroys trust instantly. IVA demos often run on mock data: demand a proof of concept with your actual core banking API before signing. Pre-built core banking connectors shorten the work; custom integration on a generic IVA platform typically takes [[fs.iva.integration]]."
      },
      { layer: 3, name: "Policy & Guardrails",
        ...
        stack: [
          { name: "Pindrop", role: "Voice Authentication & Fraud", why: "Passive voice and audio authentication replaces 'mother's maiden name' security questions and flags spoofed calls and social engineering. Pindrop says Passport saves [[fs.pindrop.aht]] of handle time per call.", href: "/vendors/acd-routing" },
        ...
        pitfall: "Voice biometrics is sold as frictionless authentication, but enrollment carries its own friction. A voiceprint needs enough of the customer's speech to build, so a customer's early calls can still fall back to knowledge-based questions or other factors. Plan for a [[fs.vb.ramp]] enrollment ramp before you see the full AHT reduction, and track your enrollment rate."
      },
      { layer: 2, name: "Workflow Execution",
        ...
        stack: [
          { name: "Pega", role: "Process Orchestration", why: "Enterprise-grade decisioning and case management used in banking. Automates Reg E dispute timelines, account opening workflows, and cross-sell next-best-action.", href: "/vendors/acd-routing" },
      ...
      { layer: 1, name: "Data Access",
        ...
        vendors: "FIS, Fiserv, Jack Henry, Temenos, Salesforce FSC", risk: "Agent desktop requiring several separate logins to serve one customer",
        stack: [
          { name: "FIS", role: "Core Banking", why: "A major U.S. core banking provider. Real-time APIs for Modern Banking Platform; batch interfaces for legacy cores. Your CX stack is only as good as your FIS integration.", href: "/vendors/payments" },
  ...
  "credit-unions": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.credit-unions.aht]]", note: "Driven by relationship-depth conversations and by generalist agents covering many products" },
      { metric: "FCR", avg: "[[fs.sv.credit-unions.fcr]]", note: "Helped by a smaller product set and deep member knowledge; limited by small specialist teams" },
      { metric: "CSAT", avg: "[[fs.sv.credit-unions.csat]]", note: "Moves with personal service and relationship continuity" },
      { metric: "Containment", avg: "[[fs.sv.credit-unions.containment]]", note: "Limited where members prefer a person for trust-sensitive requests" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
        ...
        pitfall: "Credit unions often buy analytics tools sized for large banks and then leave most of the features unused. A small CU contact center does not need the analytics depth of a national bank. Start with recording, basic QA scoring, and member satisfaction tracking: add speech analytics only when you have the staff to act on the insights."
      },
      { layer: 6, name: "Routing & Orchestration",
        ...
        stack: [
          ...
          { name: "8x8", role: "UCaaS/CCaaS", why: "Unified communications and contact center on one platform. Suits CUs where branch staff and contact center agents need to collaborate in one workflow.", href: "/vendors" },
        ],
        pitfall: "Credit unions with small contact centers often choose platforms based on price alone and end up on systems that can't do skills-based routing. When your team is small, every routing decision matters more, sending a lending question to a deposit specialist wastes the only lending expert's availability. Pay for routing intelligence even if the seat count is low."
      },
      { layer: 5, name: "Conversation Management",
      ...
      { layer: 3, name: "Policy & Guardrails",
        ...
        stack: [
          ...
          { name: "Pindrop", role: "Voice Authentication", why: "Passive voice and audio authentication reduces KBA friction, which matters for small CUs too. Pindrop says Passport saves [[fs.pindrop.aht]] of handle time per call.", href: "/vendors/acd-routing" },
        ],
        pitfall: "Credit unions share many compliance obligations with large banks but have a fraction of the compliance staff. Verafin and similar platforms solve this by automating SAR filing and transaction monitoring, but only if the CU actually integrates them with the contact center. Many CUs run compliance systems and contact center systems in parallel with no data flow between them."
      },
      { layer: 2, name: "Workflow Execution",
      ...
      { layer: 1, name: "Data Access",
        ...
        stack: [
          { name: "Jack Henry (Symitar)", role: "Core Banking", why: "A core used by many larger U.S. credit unions. Growing API ecosystem through Banno. Your CX stack's ceiling is set by how well you integrate with Symitar.", href: "/vendors" },
          { name: "Corelation (KeyStone)", role: "Core Banking", why: "API-first core used by credit unions modernizing their stack. Easier to integrate with CX platforms than legacy cores. Strong for CUs planning digital transformation.", href: "/vendors" },
        ...
        pitfall: "Symitar's Banno platform is modernizing the API layer, but many CU implementations still rely on SymXchange or PowerOn, which are batch-oriented. If your contact center integration with Symitar only refreshes member data nightly, your agents are working with stale information every morning. Push for real-time API access or accept that your CX tools will lag your core by up to a full batch cycle."
      },
    ],
  },
  ...
  "insurance": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.insurance.aht]]", note: "Driven by claims documentation and policy changes" },
      { metric: "FCR", avg: "[[fs.sv.insurance.fcr]]", note: "Multi-party coordination between policyholder, adjuster and third parties stands in the way of single-contact resolution" },
      { metric: "CSAT", avg: "[[fs.sv.insurance.csat]]", note: "Driven by the claims experience: FNOL speed and status transparency" },
      { metric: "Containment", avg: "[[fs.sv.insurance.containment]]", note: "Limited by policy complexity and the emotional stakes of claims" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
      ...
      { layer: 6, name: "Routing & Orchestration",
        ...
        pitfall: "Catastrophe routing is not a feature you configure and forget: it requires a playbook. When a hurricane hits, your contact center needs to: activate surge agents, deploy geo-targeted IVR messaging, route affected policyholders to dedicated queues, and suppress non-urgent outbound communications. Test your CAT routing before the season; a live event is the wrong time to find the gaps."
      },
      { layer: 5, name: "Conversation Management",
      ...
      { layer: 3, name: "Policy & Guardrails",
        ...
        stack: [
          ...
          { name: "Verisk", role: "Data & Analytics", why: "ISO ClaimSearch and property data for loss verification. Widely used fraud detection scores. Integrates with Guidewire for automated validation during FNOL.", href: "/vendors" },
        ],
        pitfall: "Claims handling timelines vary by state. California requires acknowledgment of a notice of claim within [[fs.ins.ack.ca]], Texas within [[fs.ins.ack.tx]] of written notice, and Florida within [[fs.ins.ack.fl]] of a claim communication. Your claims workflow must enforce each state's clock. A workflow engine that treats every state the same falls out of compliance in the strictest ones first."
      },
      { layer: 2, name: "Workflow Execution",
        ...
        stack: [
          { name: "Guidewire", role: "Core Insurance Platform", why: "Widely used claims management (ClaimCenter), policy admin (PolicyCenter), and billing (BillingCenter). Guidewire Cloud is a common target platform for P&C modernization.", href: "/vendors" },
        ...
        pitfall: "Guidewire and Duck Creek are not CX platforms: they are core insurance platforms. The contact center integration is often an afterthought in these implementations. Budget separately for the middleware that connects your claims system to your CCaaS platform. The 'we'll integrate that later' line item is where insurance CX projects often stall."
      },
      { layer: 1, name: "Data Access",
        ...
        pitfall: "Insurance is document-heavy. A single auto claim can generate many documents (FNOL, police report, photos, estimates, medical records, rental receipts). If your agent desktop can't surface these documents within the interaction, the agent has to say 'let me look that up and call you back', and first contact resolution stalls. Document access is the FCR bottleneck."
      },
    ],
  },
  ...
  "wealth-management": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.wealth-management.aht]]", note: "Driven by portfolio discussions and compliance requirements" },
      { metric: "FCR", avg: "[[fs.sv.wealth-management.fcr]]", note: "Helped by dedicated advisors who own the relationship" },
      { metric: "CSAT", avg: "[[fs.sv.wealth-management.csat]]", note: "Moves with relationship continuity and access to the named advisor" },
      { metric: "Containment", avg: "[[fs.sv.wealth-management.containment]]", note: "Limited where clients want a person for financial decisions" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
      ...
      { layer: 6, name: "Routing & Orchestration",
        ...
        pitfall: "Named-advisor routing fails when the advisor is unavailable, which can be [[fs.wm.advisor.away]] of the time (meetings, PTO, after-hours). The backup routing logic matters more than the primary. If a high-value client's advisor is out and they reach a general service representative who can't discuss their portfolio, you've damaged a relationship worth more than your entire CCaaS contract."
      },
      { layer: 5, name: "Conversation Management",
        ...
        stack: [
          { name: "Unblu", role: "Secure Collaboration", why: "Built for financial services, with wealth management use cases. Compliant co-browsing lets advisors walk clients through portfolio dashboards. Document sharing with audit trail. Video with recording and archival.", href: "/vendors/digital-engagement" },
          { name: "Symphony", role: "Compliant Messaging", why: "Built for financial services messaging. End-to-end encryption with compliance archival. Used by asset managers and banks for inter-firm and client communication.", href: "/vendors" },
      ...
      { layer: 4, name: "Reasoning & Planning",
        ...
        pitfall: "Any AI that touches investment accounts must have hard guardrails preventing investment advice. A bot that says 'based on your risk profile, you should consider reallocating to bonds' just gave investment advice without a suitability review. That is a serious compliance failure for the supervising principal. Your AI's most important capability is knowing what it cannot say."
      },
      { layer: 3, name: "Policy & Guardrails",
        ...
        pitfall: "Wealth management compliance cannot be deferred. Since December 2021 the SEC's off-channel communications initiative has charged more than 100 firms with recordkeeping failures, with penalties of [[fs.sec.offchannel]]: business conversations on personal texting and WhatsApp that were never archived. Your compliance stack must be live before your first client interaction on any channel. There is no 'phase 2' for compliance."
      },
      { layer: 2, name: "Workflow Execution",
        ...
        pitfall: "ACAT transfers (moving accounts between custodians) are an anxious stretch for new clients. FINRA Rule 11870 gives the carrying firm [[fs.finra.acat.validate]] to validate a transfer instruction and [[fs.finra.acat.complete]] after validation to complete it; exceptions, residual assets and non-standard holdings add time. If your CX stack can't provide transfer status updates automatically, your service team will spend a large share of onboarding fielding 'where are my assets?' calls. Automate ACAT status notifications before you automate anything else."
      },
      { layer: 1, name: "Data Access",
        ...
        stack: [
          ...
          { name: "Black Diamond (SS&C)", role: "Portfolio Reporting", why: "Portfolio performance reporting. Client-facing portal and advisor dashboard. The data source for any 'how is my portfolio doing?' interaction.", href: "/vendors" },
          { name: "eMoney Advisor", role: "Financial Planning", why: "Financial planning software for advisors. Integration means agents and advisors can reference a client's financial plan during service interactions, connecting 'I need to make a large withdrawal' to 'here's how that affects your retirement timeline.'", href: "/vendors" },
        ],
        pitfall: "Wealth management has a unique 'dual desktop' problem: the advisor sees the client through Orion/Black Diamond (portfolio-centric), while the service team sees the client through the CCaaS platform (interaction-centric). Neither has the other's view. The advisor doesn't see that the client called three times last week; the service team doesn't see that the client's portfolio just fell sharply. Solve this with a shared CRM layer (Salesforce FSC or Wealthbox) that both teams use."
      },
    ],
  },
  ...
  "lending-mortgage": {
    ...
    intro: "Lending and mortgage contact centers manage some of the longest customer journeys in financial services. A single mortgage can take [[fs.mtg.cycle]] from application to close, involving multiple parties, document exchanges, and regulatory milestones.",
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.lending-mortgage.aht]]", note: "Driven by rate explanations and document requirements" },
      { metric: "FCR", avg: "[[fs.sv.lending-mortgage.fcr]]", note: "Multi-party coordination (borrower, loan officer, underwriting, title) stands in the way of single-contact resolution" },
      { metric: "CSAT", avg: "[[fs.sv.lending-mortgage.csat]]", note: "Moves with pipeline visibility: borrowers who cannot see their status call to ask" },
      { metric: "Containment", avg: "[[fs.sv.lending-mortgage.containment]]", note: "Limited where borrowers want a person to confirm high-stakes steps" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
  ...
  "fintech-neobanks": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.fintech-neobanks.aht]]", note: "Driven by a simpler product set and in-app context available to the agent" },
      { metric: "FCR", avg: "[[fs.sv.fintech-neobanks.fcr]]", note: "Helped by simpler products and integrated systems; limited by dependence on BaaS and processor partners" },
      { metric: "CSAT", avg: "[[fs.sv.fintech-neobanks.csat]]", note: "Moves with speed and with whether the app can resolve the request itself" },
      { metric: "Containment", avg: "[[fs.sv.fintech-neobanks.containment]]", note: "In-app self-service carries the transactional requests; disputes and fraud stay with people" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
      ...
      { layer: 6, name: "Routing & Orchestration",
        ...
        stack: [
          ...
          { name: "Zendesk", role: "Support Platform", why: "Structured ticketing and workflow engine. Suits fintechs whose support team has outgrown chat-only queues and needs structured queue management alongside chat.", href: "/vendors" },
        ],
        pitfall: "Fintechs often start on Intercom (chat-first, simple) and outgrow it once the support team needs structured routing, WEM, and compliance features. Migrating from Intercom to a CCaaS platform mid-growth is expensive and disruptive. If you're growing fast, test whether your current platform can handle a team several times its current size before you hit the wall."
      },
      { layer: 5, name: "Conversation Management",
      ...
      { layer: 3, name: "Policy & Guardrails",
        ...
        stack: [
          { name: "Alloy", role: "Identity & Compliance", why: "Identity verification, KYC, and risk decisioning in one platform. Real-time compliance checks during onboarding and throughout the customer lifecycle. Common in fintech compliance stacks.", href: "/vendors" },
        ...
        pitfall: "Fintechs operating through bank partners (BaaS model) often assume the bank partner handles compliance. The bank partner handles banking compliance; the fintech handles customer-facing compliance. When a customer calls your support team and reports unauthorized transactions, Reg E timelines apply regardless of your bank partnership structure: the institution determines whether an error occurred within [[fs.rege.investigate]], or up to [[fs.rege.extended]] if it provisionally credits the account. Agree with your bank partner who runs that clock, and own your part of it."
      },
      { layer: 2, name: "Workflow Execution",
  ...
  "payments-processing": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[fs.sv.payments-processing.aht]]", note: "Driven by the mix: cardholder inquiries are short, merchant technical support is long" },
      { metric: "FCR", avg: "[[fs.sv.payments-processing.fcr]]", note: "Merchant issues that need escalation to technical teams stand in the way of single-contact resolution" },
      { metric: "CSAT", avg: "[[fs.sv.payments-processing.csat]]", note: "Moves with hold times during outages and with technical depth on merchant calls" },
      { metric: "Containment", avg: "[[fs.sv.payments-processing.containment]]", note: "Transaction lookups are automatable; terminal troubleshooting needs people" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
        ...
        pitfall: "Payments companies track CSAT at the interaction level but not at the merchant relationship level. A merchant can have several satisfactory interactions and still leave because settlement timing is inconsistent. Track relationship-level metrics (NPS, retention risk) alongside interaction metrics."
      },
      { layer: 6, name: "Routing & Orchestration",
        ...
        pitfall: "Terminal-down calls from merchants are the payments equivalent of an emergency call: every minute costs the merchant revenue. These calls should route directly to technical support with the highest priority, bypassing the general queue entirely. Without a 'terminal emergency' routing path, merchants wait in the general queue while their business loses sales."
      },
      { layer: 5, name: "Conversation Management",
      ...
      { layer: 3, name: "Policy & Guardrails",
        ...
        pitfall: "Adding chat, SMS, or social support to a payments contact center expands your PCI scope. Every channel where an agent could potentially see card data must be PCI compliant, including the screen recording, analytics, and archival systems that touch those channels. Processors that add digital channels without reassessing PCI scope tend to discover the gap during their next QSA audit."
      },
      { layer: 2, name: "Workflow Execution",
        ...
        stack: [
          ...
          { name: "ChargebackOps / Midigator", role: "Chargeback Management", why: "Automated chargeback response with evidence collection and network-specific formatting. Shortens the time to assemble a response.", href: "/vendors" },
        ],
        pitfall: "Chargeback response deadlines are set by the card network rules, and the acquirer passes the merchant a shorter window of its own. If your chargeback response workflow requires manual evidence collection from several systems, most of that window goes to internal process. Automate evidence aggregation or accept a lower win rate."
      },
      { layer: 1, name: "Data Access",
        ...
        stack: [
          { name: "Stripe", role: "Payment Platform", why: "API-first platform with real-time transaction data, dispute management, and merchant dashboard. Suits tech-forward processors and payment facilitators.", href: "/vendors/payments" },
          ...
          { name: "Fiserv (First Data)", role: "Processing Infrastructure", why: "Large card processor. Clover terminal ecosystem. Agent desktop needs real-time access to Fiserv authorization data, settlement status, and merchant portfolio.", href: "/vendors/payments" },
        ],
        pitfall: "Payments data is fragmented by design: the acquirer, processor, gateway, and card network each hold different pieces of the transaction lifecycle. An agent answering 'why was this transaction declined?' may need to check the gateway (technical decline), the processor (risk decline), and the issuer (insufficient funds): all in different systems. Build a unified transaction view that shows the full lifecycle in one screen, or accept that every lookup takes agents several minutes."
      },
    ],
  },
};
