...
| 2026-09-23 (S23) | Phase C step 1: Transformation Readiness on the rubric engine | `src/lib/rubrics/transformationReadiness.js`, published at /methodology/transformation-readiness (sitemap 427). 24 statements with actions, a next diagnostic per dimension, limits. Legacy cut points kept; the first band starts at the scale floor (unreachable below 1). The Close this gap and Monitor flags, previously unpublished, are stated on their bands and proven equal on 20,000 answer sets. Technical Readiness routes to Platform Decision; Strong band copy softened. `rubric.test.mjs` 384. Suite 19,829 green; local live check 196 of 196 |
| 2026-09-23 (S23) | Phase C step 2: CX IT Alignment on the rubric engine | Paired kind `scorePaired` (engine markers): gap per pair, area mean, weighted overall; legacy gap bands kept; 20,000 answer sets equal the legacy formula for overall, band and every area gap. Published rule (TB approved): 2+ points apart is misaligned, both sides at 2 or below is a shared weakness (the case a gap of 0 hid); each pair has an align and a build action. Rubric page renders paired rubrics; /methodology/cx-it-alignment (sitemap 428). Two-respondent guidance via the scenario link. False "profile has been saved" copy removed from CX IT and Governance. `rubric.test.mjs` 430. Suite 19,880; local live check 197 of 197 |
| 2026-09-24 (S23) | Phase C step 3: Governance & Operating Model on the ownership engine | `src/lib/ownership.js` and the published model `src/lib/rubrics/governance.js` (TB approved): accountable plus one contributor, no score, six rules with published thresholds and severities; Risk & Compliance appended as a sixth role (old links keep every assignment); the unpublished overload flag (more than 8) replaced by twice an even share (10 of 30). Model page at /methodology/governance-model (sitemap 429), rendered from the same object. `ownership.test.mjs` 46: every rule against an independent oracle on 20,000 maps, legacy item list and counts frozen, hostile maps. Suite 19,931; local live check 198 of 198 |
| 2026-09-24 (S23) | Live checker hostile outcomes; QA Scorecard defects | `scripts/live-check.mjs`: a hostile link now waits for a result, a start screen or a blocked-result notice and reports which, so no hostile PDF check is skipped silently (205 of 205 locally). QA Scorecard: the evaluation's Yes and No buttons never showed as selected (fixed, `aria-pressed`); a partly marked evaluation printed a score as if unmarked criteria passed, now no score until every criterion is marked, with a count; the 85 and 70 bands are labelled planning defaults with no published source. How to Choose QA description corrected. Suite 19,931 green. Calibration module and form checks go to TB as a design first. |
| 2026-09-24 (S23) | Phase C step 4: QA Scorecard on its own engine | `src/lib/qa.js` and the published model `src/lib/rubrics/qaScorecard.js` (TB approved). Form checks (weights, empty category, definitions, auto-fail reasons, 15-point swing heuristic, focus mix as a fact). Blind calibration by submission code; nothing compared until every evaluator has scored every call. Center of CX Calibration Method 1.0: Krippendorff's alpha (interval scores, nominal marks), Gwet's AC1 on critical fails, percent agreement, seeded bootstrap intervals, Krippendorff 2004 bands; labelled heuristics for bias, spread, item agreement and critical load; optional reference evaluator. Next step order: form, calibrate, recalibrate, FCR Leakage. `/methodology/qa-scorecard` (sitemap 430). `qa.test.mjs` 97 checks with published fixtures; suite 20,033. Found: ReportExport renders section text as raw HTML (carried debt). |
| 2026-09-24 (S23) | Security: report renderer escapes every field | `ReportExport.jsx` built the report window from raw section strings, so a crafted scenario link could inject markup into user text that ran in the same-origin window. The HTML builder is now the pure `reportHtml`, escaping every section and cover field; a scan of 37 reports across all 25 tools found no intended markup. QA's own escaping removed so nothing is escaped twice. `export.test.mjs` (11) attacks each section type and checks every interpolation. Suite 20,044. |
| 2026-09-24 (S23) | Security hardening; QA calibration flow | TB: a complete solution for every security finding. `SECURITY.md` protocol (confirm, fix at the sink, sweep, second layer, gate, verify on production, record). Site-wide CSP and security headers in `vercel.json`; report window policy `script-src 'none'`, print wired from the site, opener cut. `security.test.mjs` (29). Live checker fails on any policy violation and checks served headers; all 430 sitemap pages and 209 live checks clean under the policy. QA: four-step session guide, evaluator link opening at step 3, add-my-code, sample session, rejection guidance. |
| 2026-09-24 (S23) | Phase C step 5: Platform Decision as the renewal gate | `src/lib/renewal.js` and `src/lib/rubrics/platformDecision.js` (TB approved). Rating, evidence and need per item; no averaging; unknown is a proof request; layer outcomes renew, conditions, specialist, market test; gate renew, conditions, evaluate; renewal clock with labelled heuristics; negotiation checklist. Legacy sample surfaces 14 must-have gaps the average hid. Unpublished Stay/Extend/Evaluate/Replace cut points, the Vendor Match ranking promise and the 'scored IVA vendors' label retired. `/methodology/platform-decision` (sitemap 431). `renewal.test.mjs` 83. Suite 20,161. |
| 2026-09-24 (S23) | Phase C steps 6 and 7: Contract Risk and RFP Builder | Contract Risk: 13 clauses with published severities and reasons, don't know as a find item, published reading rule, unsourced figures retired, `terms.test.mjs` 77. RFP Builder: published requirement set and weights, response scorer (GA-only full credit, unanswered never zero, demo verification), analyst read on the buyer's data, order only among entered vendors with ties, Vendor Match bridge as a starting list, lead path kept; fixed vertical tags lost on links; packed scenario links. `rfp.test.mjs` 62. `/methodology/contract-risk`, `/methodology/rfp-builder` (sitemap 433). Suite 20,320. |
| 2026-09-24 (S23) | Phase D step 1: Occupancy Risk to V3-Full | Engine with injected, registered constants; one labelled attrition model (invented 0.15 formula retired); shared occupancy bands; target input; loaded staffing cost; BLS wage default; training ramp in replacement cost; unused tenure removed; unsourced claims retired; meaningful opening case. First generated calculator method page with an engine-computed worked example (`/methodology/occupancy-risk`, sitemap 434). `occupancy.test.mjs` 34 (oracle, A/B, laws, fixture, mutants), `occupancy.report.mjs` 34 (PDF reconciliation). Suite 20,393. TB actions closed: PostHog is the event source (Vercel Hobby has no custom events); `suite` required on main. |
| 2026-09-24 (S23) | Phase D step 2: Shrinkage Planner to V3-Full | TB approved six calls. Engine `src/lib/shrinkage.js` (markers, constants injected). PTO counted as planned (was unplanned). New gross-up: agents to schedule = need ÷ (1 − shrinkage), rounded up, beside agents on the queue. "Annual Cost of Gap" retired: paid time off the queue priced at the loaded BLS wage over the shared 2,080-hour year, split planned and unplanned, stated as wages already paid and no saving; one shrinkage point sized. Retired: "industry norm", "28-35% typically", "3-5 points highest-ROI", "Well managed", the $18 wage. The 28 to 35% range is one shared labelled heuristic (`shrinkage.range.low/high`, equal to `BENCH.shrinkage`); Staffing's flag text now calls it a planning range. Paid hours moved to shared definitions `time.hours.week` and `time.hours.year` (Occupancy repointed). Sums tidied to a millionth of a point, so addition order cannot move a total across the range. Found in the visual check: the range printed as 28.000000000000004; fixed on page and method page, and float noise is now bad text in `shrinkage.report.mjs`, `floor.test.mjs` (every tool, default and sample; no sibling found) and the live checker. Chunk gate re-based with module attribution (entry growth since 11A is method page metadata and routes, redirects, lazy retry, error boundary, `type.js`; no data file or route component). Published at /methodology/shrinkage-planner (sitemap 435). `shrinkage.test.mjs` 34 (oracle 20,000; A/B 10,000 with only exact half-agent cases the old float noise rounded down differing; laws; fixture; 4 mutants), `shrinkage.report.mjs` 44. Suite green; local live check under the production policy 214 of 214. |
| 2026-09-24 (S23) | Phase D step 3: AHT Decomposition to V3-Full | TB approved six calls. Engine `src/lib/aht.js` (markers). "Reducible time" and "all reducible without cutting talk time" retired; the tool states time outside the conversation as a fact. Unsourced share ranges (55 to 65% talk and so on) and reducibility ratings retired. The seven initiative reduction factors registered as labelled heuristics (`aht.lever.*`), opened as editable shares; a lever counts only when selected; selected levers compound on a shared component (the previous sum double counted 55 × 15% × 10% of hold). The 90% talk floor never bound in 10,000 cases and is gone. Contacts per month turns seconds into agent hours a year, labelled capacity. Colour thresholds (38, 45) removed, presets labelled illustrative, the "The goal is not" cadence reworded. Published at /methodology/aht-decomposition (sitemap 436). `aht.test.mjs` 35 (oracle 20,000; A/B 10,000: every lever equal to the previous scenario to float precision, 26 of 40,000 printed values differ, each an exact rounding line the previous float noise rounded the other way; floor never bound; the combined difference is exactly the double counted hold; laws; fixture; 4 mutants), `aht.report.mjs` 39. Suite 20,596 green; local live check under the production policy 215 of 215. |
| 2026-09-24 (S23) | Phase D step 4: Forecast Accuracy to V3-Full | TB approved six calls. Engine `src/lib/forecast.js` (markers). Interval accuracy (1 minus WAPE) is the headline: the old "overall accuracy" was total-volume accuracy, in which interval errors cancel, so a forecast off 20% every interval scored 99.8% and printed "Excellent" (pinned in the harness; WAPE reads 20.1%). MAPE shown beside WAPE; total-volume accuracy kept as a line with the contacts that cancelled. Misses ranked by contacts. The unsourced 3% bias line replaced by the tracking signal against the textbook plus or minus 4 limit (`forecast.ts.limit`, threshold, cited). Grades and claims retired (95/90, 5/10, Excellent, Best-in-class, "not a better algorithm", the 10% interval colours). Optional AHT turns misses into workload hours and agents busy per 30-minute interval. `CalcPage` takes an example note; this page's example is two intervals that cancel. Published at /methodology/forecast-accuracy (sitemap 437). `forecast.test.mjs` 32 (oracle 20,000; A/B 10,000: total-volume accuracy, MAPE and bias equal the previous tool; laws: no cancelling, scale free, sign flip; fixture; 4 mutants), `forecast.report.mjs` 37. Suite 20,670 green; local live check under the production policy 216 of 216. |
| 2026-09-25 (S23) | Phase D step 5: Schedule Adherence to V3-Full; Phase D engines complete | TB approved six calls. Engine `src/lib/adherence.js` (markers). Opening case moved to 820 calls an hour: the old default ran at 23% occupancy and printed 100% service level at every adherence. The stepped abandonment heuristic (1/2/4/8/15% by speed of answer) retired. Overtime now prices the agents needed to hold the target (fewest on the queue meeting it, divided by adherence, rounded up, beyond the roster) over entered open hours and days; it used to price every missing agent, including today's, as a full 8-hour shift for 250 days. Overtime multiplier registered as the FLSA minimum (`adh.ot.multiplier`, market, 29 U.S.C. 207); BLS wage; met or missed against the target replaces the unsourced amber line. The target solver continues the Erlang B recurrence (a million calls an hour solves in 0.1 s). Erlang C pinned to published tables (10 Erlangs on 11 to 14 agents: 0.6821, 0.4494, 0.2853, 0.1741) and to a direct-sum oracle. Found: Staffing's solver starts at ceil(A)+1 and can skip floor(A)+1 on a fractional load, overstating by one agent when that count already meets the target (4 of 3,000 random queues); recorded for the rail step. Published at /methodology/schedule-adherence (sitemap 438). `adherence.test.mjs` 37, `adherence.report.mjs` 35. Suite 20,747 green; local live check under the production policy 217 of 217. |
| 2026-09-25 (S23) | Phase D rail step: WFM tools into Staffing; Phase D complete | TB approved five calls. AHT Decomposition publishes today's handle time (never the post-lever figure), the Shrinkage Planner total shrinkage, Occupancy Risk its target as Staffing's occupancy ceiling, each with an origin grade: Directional at an example or corrected input, Planning-grade once the reader's own (no document path, so never Finance-grade). Staffing reads the three once at mount through getExternalWithSource; a scenario link outranks the rail (D13); each pulled field is badged PULLED and names its source until edited; while a field holds the pulled value its driver grades by railEvidence(origin) and is never compared with the operating profile; the PDF names the source. Forecast and Adherence publish nothing to Staffing. Staffing's solver now starts at floor(A)+1: A/B on 20,000 random queues differs in 1,297, each a fractional load where floor(A)+1 already meets the target and the old start answered one more (about 0.2% of queues at 70 to 90% targets). The Adherence harness now requires exact agreement with Staffing. Rail audit fix: a member access inside a publish call (v.target) no longer credits every key of v (it listed Occupancy Risk as an aht, agents and attritionRate publisher); proven by a control mutant. Rail pins M2, M3, M10, M11 updated to the new facts. `wfmrail.test.mjs` 34 (publishers, round trips with provenance, grading, scenario precedence, solver A/B). Suite 20,783 green; rail audit clean; local live check 217 of 217; browser check of the chain (AHT 390 Planning-grade, shrinkage 33% and ceiling 85% Directional, three PULLED badges; fresh tab none). |
| 2026-09-25 (S23) | Phase E1: published method pages for the nine rail calculators | Staffing, Cost per Contact, Channel Shift, FCR Leakage, AI Deflection, TCO, License Gap, Attrition and Business Case each publish a method page at /methodology/<tool> (sitemap 447): formulas in words, bands, every constant from the registry with its kind and source, a worked example, limits. Engines stay in the tool files; each page carries its example as pins and `methods.test.mjs` (101) recomputes every pin from the tool's own engine, checks routing, sitemap, SEO, registry equality and that each tool page and PDF links its method. Staffing also pins Nextiva's published Erlang C case (68 base, 98 FTE, 84.0%). Found on the way and fixed: Attrition and Business Case read no registry. Every constant is now registered (`attrition.*` 34, `bcb.*` 37), and both open on the shared BLS wage and benefits load (J10, J11): Attrition salary $38,000 to $42,827, load 28% to 30%; Business Case wage $18 to $20.59. A/B on 20,000 cases each: equal inputs give equal figures and grades. Fixtures stay pinned at their verified inputs; Business Case still opens on the tracker fixture (net $31,850, three-year cost $1,722,000). Unsourced copy retired: Attrition's "published 40-60% band" and "$10-20K all-in reference" (now a labelled planning band), Business Case's "most centers actually achieve" and "15 to 25% is realistic" (now internal planning ranges). `adh.ot.multiplier` is shared (Attrition's overtime premium). Suite 20,934. |
| 2026-09-25 (S23) | Phase E2: reference fixtures | `src/lib/fixtures.js` with seven cases known outside the site (Erlang C tables, a hand derivation, the 80/20 textbook case, Nextiva, Krippendorff 2011, a cancelling WAPE case, the tracker fixture), each computed by the shipped engine in `fixtures.test.mjs` (16) with an independent Erlang C oracle; shown on method pages under Checked against. |
| 2026-09-25 (S23) | Phase E3: method version stamps and public changelog | Every tool with a published method stamps "Method 1.0, published ..." on the page and the PDF cover (ReportActions, `methodVersions.js`, equal to every model). Private version strings in AI Deflection, Staffing and TCO retired. `/changelog` lists method changes only, from the Phase D rebuild on, and each method page lists its own. Sitemap 448. Chunk gate re-based with attribution. Suite 20,968; live check 227 of 227. |
| 2026-09-25 (S23) | Full site scan, part 1: dashes and retired language (13-03 closed) | Every em and en dash in the repository removed: 1,437 across 53 files, mostly the ten sub-vertical data files. Rules: numeric ranges read "5 to 10"; a dash after a short label becomes a colon; a pair around an aside becomes commas; before a conjunction, relative word or preposition a comma; otherwise a colon; placeholders a word or a dot. Hand fixes where a rule read badly. Superlatives retired in public copy (41): industry-leading, best-in-class, world-class, seamless, robust, industry-standard vendor praise (now plain descriptions or "widely used"). New `copy.test.mjs` gates both on every tracked text file, each rule proven to fire. Suite 20,973; local live check 227 of 227. |
| 2026-09-25 (S23) | Full site scan, part 2: integrity freeze extended to every category (TB: yes) | Phase 1 scores, tiers, ranks, quadrants, leaderboards, fit ratings and score order no longer render for IVA, Agent Assist, WFM and QM, Analytics, Digital Engagement, Payments or ACD and Routing. The seven category pages share `src/lib/Phase1Directory.jsx`: a withdrawn-scores banner and a directory grouped by each category's own taxonomy, by name, one neutral classification line, the Phase 1 label. The seven profile branches in `VendorProfile.jsx` show a Phase 1 badge in place of the score badge; dimension grids, rank, leaderboard, tier and quadrant removed. `researchStatus(category, slug)`, `phase1Label()` and `byName` in `researchStatus.js`. CCaaS freeze gap closed: the ten industry pages printed Phase 1 CCaaS scores (Genesys 94) and ordered by them; now name order, no number, adjacent label kept. "Scored" claims removed from SEO (seven category titles now "N Vendors Profiled"), Industries, Research, Vendors, CX Ecosystem, `index.html` and the vendor profile template; tools hub count now derived (said 30, is 25). Both Phase 1 buyer guides carry a Phase 1 edition notice; their content and PDFs wait on TB. Profile nav no longer shows Vendors twice. `freeze.test.mjs` section 7 gates all of it, three planted regressions caught. Suite 21,022; live check 227 of 227. |
| 25 Sep 2026 (S23) | Full site scan, part 3 (TB approved three calls) | CCaaS guide page tiers and scoring model removed; PDFs kept as dated Phase 1 editions. Industry-page statistics 60 to 16, each checked against the primary publisher, aggregator and vendor-blog sources removed, gated in `copy.test.mjs` section 4. Unsourced tool figures (AI Deflection, FCR, BCB, TCO) relabelled and registered; TCO A/B neutral on 12,000 cases. `ToolGate.jsx` deleted. "X, not Y" cadence deferred to the aesthetic rebuild copy pass. Open: sub-vertical prose figures. Suite 21,083; local live check 227 of 227. |
| 25 Sep 2026 (S23) | Industry sub-pages ungated (TB: yes) | The 61 stack frameworks opened only after a work email posted to Formspree, so visitors and search engines saw the gate alone. They now open on the framework; reaching the results sends nothing; Copy my profile keeps it; an optional labelled review request is the only send and confirms only on success. False "profile has been saved" removed. KPI grid and capability rows fit a phone. Browser sweep 122 runs clean (61 pages, phone and desktop). `copy.test.mjs` section 5. |
| 25 Sep 2026 (S23) | Full site sweep closed; new priority list; TB decisions | TB decided: TCO savings at `load.marginal` 1.18 with a disclosure line (Path B); BCB baseline evidence as one graded question; Market Position Index Path 3 (class-scoped ranking plus an unordered category-wide map); CCaaS-by-industry pages rebuilt at Stage 3 and noindexed until then; omni-channel distribution on free channels; design in a separate chat briefed by `docs/DESIGN_HANDOFF.md`. Priority list P0 to P7 in CLAUDE.md section 11: Healthcare research first (new session), then prerender, structured data and share cards, then measurement, then the two engine decisions. |
| 25 Sep 2026 (S23) | Healthcare research and originality pass (site scan part 4) | Every figure on the Healthcare page, hub card and six sub-pages is now a fact read on the publisher's page, a labelled planning assumption, or "No public benchmark". Benchmark table rebuilt: most cells were SQM all-industry figures in healthcare cells and the 52% FCR contradicted SQM's 69% health insurance figure; top quartile removed. HIPAA penalty updated to the 2025 adjustment ($145 to $73,011); Medicare Advantage grievance timing corrected to 42 CFR 422.564. Two PubMed facts added, seven unsourced figures retired, four relabelled. Originality check: no copied run of 8 words, three paraphrases rewritten. `claims.test.mjs` green. |
| 25 Sep 2026 (S23) | Industry research and originality pass, nine industries (site scan part 4) | One shared sub-page component. Every figure on the ten industry pages, 61 sub-pages and hub cards is a sourced fact, a labelled assumption, an example, or no public benchmark. Benchmark tables rebuilt on SQM's own figures; top quartile columns removed. Regulatory corrections across FTC, PHMSA, NHTSA, FERPA, Section 508, EU261, state claim rules and FCC. CMS test-call strip on the health insurance page. Originality: no copied run of 8 words; phrase search coverage recorded per page where the search budget ran out. BLS May 2025 wage update scheduled as its own change. Suite 23,870; 144 browser runs clean. |
| 25 Sep 2026 (S23) | P1 reach foundations: prerender, structured data, share cards, noindex | Every sitemap page ships its body in the HTML and hydrates (a tool page with a scenario link or session state renders fresh). Structured data by page type from one function; industry pages cite every source they render; the invisible TCO FAQ retired. A share card per tool, method and industry page, self-hosted. All 80 category-by-industry pages noindex and out of the sitemap (426 URLs). Found and fixed: style text escaping broke hydration on every page; nested links on card pages. `prerender.test.mjs` 39; live check 254 of 254 locally. |
| 25 Sep 2026 (S23) | P2 measurement: taxonomy freeze and 3-02 NextDiagnostic | PR #37 merged (P0, P1, task 7) and checked on production. Event taxonomy 1.0 frozen with a session landing event and docs/MEASUREMENT.md (UTM convention, five PostHog funnels incl. 11-04). One next step per result from journey.js on the page and in the PDF; 25 tool-authored next lists removed (several had drifted from the graph); engine tools pass their choice; every engine choice is a graph edge. Suite 24,024. |
| 25 Sep 2026 (S23) | P3 task 9: BCB baseline evidence (Business Case method 1.1) | PR #38 merged (889c188); one next step checked on production and the nightly triggered. New question "Where do your baselines come from?" for handle time, FCR, contact volume and wage: our defaults Directional, your estimate or an unattested system report Planning-grade, an attested system report Finance-grade. A baseline pulled from another tool grades by its origin (`railEvidence`) while it holds the pulled value; an unanswered question with a baseline edited reads as your estimate (old links). The benefit stream is the weaker of the attribution caps and the baseline grade. A/B on 6,000 cases: every figure, the cost stream, realization and completeness identical; evidence equals the weaker of the previous evidence and the baseline grade. `bcb.test.mjs` 12i (41), report sets pinned. Method 1.1 and changelog entry. Suite 24,060; local live check 254 of 254. |
| 25 Sep 2026 (S23) | P3 task 10: TCO marginal load (TCO method 1.1) | PR #39 merged (73e3a60), baseline question checked on production, nightly triggered. TCO `marginalPerContact` values handle-time labor at wage × `load.marginal` 1.18, never above the loaded rate entered (TB Path B, J10). Containment and FCR savings move; every cost, unit cost, grade, and the AHT and attrition levers unchanged (A/B on 6,000 cases in `tco.test.mjs` against the engine at the old load; one-off 12,000). Disclosure line on page and PDF: capturing the saving by not backfilling seats removes benefits too, about 10% more. Opening case marginal $2.75 to $2.51; savings $71K to $68K gross a month. Pins, method page, changelog. Open for TB: AHT lever and BCB derived marginal still at the loaded rate. Suite 24,074; local live check 254 of 254. |
