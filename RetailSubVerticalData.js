...
export const retailSubVerticals = {
  "ecommerce-dtc": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.ecommerce-dtc.aht]]", note: "Driven by the share of transactional contacts (order status, returns)" },
      { metric: "FCR", avg: "[[retail.sv.ecommerce-dtc.fcr]]", note: "Moves with product set complexity and whether agents can act in the order system" },
      { metric: "CSAT", avg: "[[retail.sv.ecommerce-dtc.csat]]", note: "Moves with returns friction and shipping delays" },
      { metric: "Containment", avg: "[[retail.sv.ecommerce-dtc.containment]]", note: "Depends on bots reading live order, carrier and returns data" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
        ...
        stack: [
          { name: "Gorgias", role: "eCommerce Analytics", why: "Revenue attribution built into the support platform. Reports revenue each agent generates through order changes, upsells and save offers, so support can be measured as a revenue source.", href: "/vendors" },
          { name: "NICE Nexidia", role: "Interaction Analytics", why: "Speech and text analytics at scale. Identifies systemic issues: when many customers mention sizing in one week, the fix belongs on the product page. Root cause detection across channels.", href: "/vendors/analytics" },
        ],
        pitfall: "eCommerce brands track CSAT at the ticket level but not at the customer journey level. A customer who contacts support several times about the same return, and gets a good answer each time, can rate every interaction well and still leave a poor review of the brand. Track resolution across the journey as well as each interaction."
      },
      { layer: 6, name: "Routing & Orchestration",
        ...
        stack: [
          ...
          { name: "Gorgias", role: "eCommerce Helpdesk", why: "Shopify-native with order data visible in every conversation. Macros with order variables (tracking link, refund status) let agents close common tickets in one click.", href: "/vendors" },
          { name: "Genesys Cloud", role: "Enterprise CCaaS", why: "For DTC brands growing into large agent teams or adding voice and complex routing. Predictive routing matches customer intent to agent skill. More platform than a small Shopify store needs.", href: "/vendors/genesys" },
        ],
        pitfall: "Many DTC brands route by channel (chat queue, email queue, phone queue) instead of by intent. A pre-purchase question on chat ('does this come in blue?') and a complaint on chat ('this arrived damaged') go to the same agent. The pre-purchase question is a revenue opportunity; the complaint is a retention risk. They need different agent skills. Route by intent, not by channel."
      },
      { layer: 5, name: "Conversation Management",
        ...
        stack: [
          ...
          { name: "Klaviyo", role: "Proactive Communications", why: "Post-purchase email and SMS flows: shipping confirmation, delivery updates, review requests. Well-built flows answer the order status question before the customer asks it.", href: "/vendors" },
          { name: "Loop Returns", role: "Self-Service Returns", why: "Self-service returns portal that lets customers start most returns without an agent. Exchange-first logic offers exchanges before refunds.", href: "/vendors" },
        ],
        pitfall: "DTC brands deploy proactive shipping notifications but send them with the carrier's generic tracking page, which can show 'in transit' for days with no update. The customer sees the notification, clicks the link, sees no progress, and contacts support anyway. Use a branded tracking page (Narvar, AfterShip, Malomo) with estimated delivery, delay explanations, and self-service options. For many customers the tracking page is the whole support experience. If a shipment will miss the date promised, the FTC Mail Order Rule requires the seller to seek the customer's consent to the delay or refund; with no date stated, the seller needs a reasonable basis to ship within [[retail.ftc.mailorder]]."
      },
      { layer: 4, name: "Reasoning & Planning",
        ...
        pitfall: "WISMO bots are an easy win in eCommerce, but only if they return real-time data. A bot that says 'your order is on its way!' when the package has sat at a carrier facility for days makes things worse. Connect your bot to the carrier API (not your OMS) for last-mile tracking accuracy. Your OMS knows the order shipped; the carrier API knows where it actually is."
      },
      ...
      { layer: 2, name: "Workflow Execution",
        ...
        stack: [
          { name: "Loop Returns", role: "Returns & Exchanges", why: "Exchange-first returns platform. When a customer initiates a return, Loop offers exchanges, store credit, and variant swaps before showing the refund option. Keeps part of the return value as an exchange or credit.", href: "/vendors" },
        ...
        pitfall: "The default returns workflow for many brands is: customer requests return → agent approves → refund issued. This is a revenue leak. Exchange-first returns (Loop, Narvar) present the customer with exchange options, store credit with a bonus, and variant swaps before showing the refund button. Measure how much refunded value your own exchange-first flow keeps; it is often one of the cheapest workflow changes in eCommerce CX."
      },
      { layer: 1, name: "Data Access",
        ...
        stack: [
          { name: "Shopify", role: "Commerce Platform", why: "Widely used by DTC brands. Order data, customer data, product catalog, and inventory in one platform. Your CX stack starts with Shopify integration, if your helpdesk can't pull Shopify data natively, you're building workarounds.", href: "/vendors" },
  ...
  "omnichannel-retail": {
    ...
    intro: "Omnichannel retail contact centers manage the intersection of digital and physical commerce. The CX challenge is continuity: a customer who buys online and picks up in store, returns in a different store, and contacts support via chat expects every touchpoint to have full context. The reality is usually fragmented systems, siloed inventory, and agents who can see the online order but not the in-store return.",
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.omnichannel-retail.aht]]", note: "Driven by lookups across POS and ecommerce systems for cross-channel issues" },
      { metric: "FCR", avg: "[[retail.sv.omnichannel-retail.fcr]]", note: "Cross-channel issues often need store-level coordination" },
      { metric: "CSAT", avg: "[[retail.sv.omnichannel-retail.csat]]", note: "Moves with the gap between the omnichannel promise and siloed systems" },
      { metric: "Containment", avg: "[[retail.sv.omnichannel-retail.containment]]", note: "Limited where questions need store-specific knowledge" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
      ...
      { layer: 6, name: "Routing & Orchestration",
        ...
        stack: [
          ...
          { name: "Genesys Cloud", role: "Enterprise CCaaS", why: "Skills-based routing with store knowledge attributes. Routes BOPIS issues to agents familiar with that store's operations. Handles the routing complexity of large store estates.", href: "/vendors/genesys" },
        ],
        pitfall: "Omnichannel routing requires store-level context that many CCaaS platforms don't natively support. When a customer calls about a BOPIS pickup at a specific store, the agent needs to know that store's hours, pickup location, and current wait time. This data lives in the retail operations system, not the CCaaS. Build a store information layer that surfaces location-specific data in the agent desktop."
      },
      { layer: 5, name: "Conversation Management",
      ...
      { layer: 2, name: "Workflow Execution",
        ...
        pitfall: "BOPIS success depends on the store fulfillment team, not the contact center, but the contact center absorbs the complaints when it fails. 'I got a pickup notification but the store says it's not ready' is a common BOPIS complaint. The fix is operational (improve store pick times and notification accuracy), but the contact center needs real-time visibility into store fulfillment status to give honest answers instead of 'let me check with the store.'"
      },
      { layer: 1, name: "Data Access",
        ...
        stack: [
          ...
          { name: "Oracle Retail", role: "Enterprise Retail Platform", why: "Full retail suite: merchandising, supply chain, POS, and customer engagement. Common in large-format retail. Deep inventory management across thousands of locations.", href: "/vendors" },
        ],
        pitfall: "A core omnichannel data problem: the online customer and the in-store customer are different records. A customer who buys online with email@gmail.com and swipes a loyalty card in store with a phone number exists as two people in many retail systems. Without identity resolution linking these records, the agent sees half the customer's history. Invest in customer identity resolution (Amperity, Treasure Data) before claiming 'unified customer view.'"
      },
    ],
  },
  ...
  "subscription-membership": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.subscription-membership.aht]]", note: "Driven by retention conversations that need exploration and negotiation" },
      { metric: "FCR", avg: "[[retail.sv.subscription-membership.fcr]]", note: "Skip, pause and modify requests are usually self-contained" },
      { metric: "CSAT", avg: "[[retail.sv.subscription-membership.csat]]", note: "Moves with the share of contacts from customers who have already decided to cancel" },
      { metric: "Containment", avg: "[[retail.sv.subscription-membership.containment]]", note: "Skip, pause and modify can be automated; cancellation conversations need a person" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
        ...
        stack: [
          ...
          { name: "Brightback (Chargebee Retention)", role: "Cancellation Analytics", why: "Cancellation flow analytics showing where and why subscribers cancel. Tests different retention offers and measures save rates by segment.", href: "/vendors" },
        ],
        pitfall: "Subscription brands measure churn rate but not the mix of churn drivers. [[retail.ex.churn-drivers]] Each driver requires a different retention strategy. Price churners need offers; product fatigue churners need variety; service churners need experience fixes. Track the driver, not just the rate."
      },
      { layer: 6, name: "Routing & Orchestration",
        ...
        pitfall: "Retention routing is only effective if retention agents have authority. [[retail.ex.save-offer]] If the customer has already decided to leave, a small fixed offer can feel dismissive. Give retention agents a tiered offer menu based on customer LTV and churn risk score."
      },
      { layer: 5, name: "Conversation Management",
        ...
        stack: [
          { name: "Recharge", role: "Subscription Management", why: "A common choice for Shopify subscription management. Skip, pause, swap, and frequency changes through self-service portal. Reduces cancellation by making modification easy.", href: "/vendors" },
          ...
          { name: "Brightback (Chargebee)", role: "Cancellation Experience", why: "Intelligent cancellation flow that presents tailored alternatives based on the cancellation reason. 'Too expensive?' → offer discount. 'Too much product?' → offer reduced frequency.", href: "/vendors" },
        ],
        pitfall: "The FTC's 2024 'click to cancel' amendments to the Negative Option Rule are now [[retail.ftc.clicktocancel]], and the FTC has asked whether to bring parts of them back. The Restore Online Shoppers' Confidence Act still requires online sellers to provide [[retail.ftc.rosca]] to stop recurring charges, and the FTC keeps enforcing against hard cancellation. Dark pattern flows (hidden cancel buttons, mandatory phone calls, repeated confirmations) carry regulatory risk and brand damage. The winning strategy isn't making cancellation hard: it's making modification so easy that customers pause or downgrade instead of cancelling."
      },
      { layer: 4, name: "Reasoning & Planning",
      ...
      { layer: 3, name: "Policy & Guardrails",
        ...
        pitfall: "Failed payments drive much of involuntary subscription churn. Many brands retry the same card a few times and then cancel the subscription. Smart dunning (Stripe, Chargebee) retries at optimal times, tries alternative payment methods, and sends the customer payment update reminders before cancelling. Compare recovered revenue before and after the change to see what it is worth to you."
      },
      { layer: 2, name: "Workflow Execution",
        ...
        pitfall: "Many subscription brands have a cancellation workflow but not a win-back workflow. A customer who cancels in March might be winnable in June with the right offer, but only if you have a timed, segmented re-engagement sequence. Build a win-back flow triggered by cancellation, with offers at set intervals that escalate over time. A former subscriber already knows the product, which a cold prospect does not."
      },
      { layer: 1, name: "Data Access",
        ...
        pitfall: "Subscription data lives in Recharge; order data lives in Shopify; payment data lives in Stripe; engagement data lives in Klaviyo. Your agent needs all four to handle a subscription conversation effectively. Many helpdesks integrate Shopify but not Recharge, so the agent can see past orders but can't see the active subscription, billing schedule, or modification history. Integrate the subscription platform first; subscription conversations depend on it more than on any other source."
      },
    ],
  },
  ...
  "marketplace": {
    ...
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.marketplace.aht]]", note: "Driven by disputes that need investigation and coordination between parties" },
      { metric: "FCR", avg: "[[retail.sv.marketplace.fcr]]", note: "Disputes and policy issues often need investigation and follow-up" },
      { metric: "CSAT", avg: "[[retail.sv.marketplace.csat]]", note: "Moves with dispute outcomes: one side loses every dispute" },
      { metric: "Containment", avg: "[[retail.sv.marketplace.containment]]", note: "Order status can be automated; disputes and policy questions need a person" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance",
      ...
      { layer: 6, name: "Routing & Orchestration",
        ...
        pitfall: "Marketplace seller support is B2B support, but many marketplaces staff it like B2C. A seller whose payments are on hold has a cash flow crisis; treating their case with the same SLA as a buyer asking about a tracking number destroys seller trust. Build a separate seller support path with B2B-grade SLAs, especially for payment and account health issues."
      },
      { layer: 5, name: "Conversation Management",
        ...
        stack: [{ name: "Custom / Zendesk", role: "Marketplace Support", why: "Many marketplaces build custom buyer-seller messaging to control the experience and prevent off-platform transactions. Zendesk sits behind it for case management and agent tooling.", href: "/vendors" }],
      ...
      { layer: 4, name: "Reasoning & Planning",
        ...
        stack: [{ name: "Ada", role: "AI Agent", why: "Handles buyer WISMO and seller FAQ at scale. Routes disputes to human agents with pre-collected evidence from both parties. Agents start each dispute with the evidence already gathered.", href: "/vendors/iva" }],
      ...
      { layer: 2, name: "Workflow Execution",
        ...
        vendors: "Salesforce, custom platforms, Stripe Connect", risk: "Disputes dragging on for weeks and losing both buyer and seller",
        ...
        pitfall: "Dispute resolution workflows must have hard deadlines or they languish. Set an evidence window, a review target and a total resolution target, for example [[retail.mkt.dispute]]. Disputes that drag on lose both the buyer and the seller whatever the outcome."
      },
      { layer: 1, name: "Data Access",
        ...
        pitfall: "Marketplace agents need to see both sides of every transaction, the buyer's history AND the seller's history, to make fair dispute decisions. Many marketplace support tools show one side at a time, forcing agents to switch between buyer view and seller view. Build a dispute resolution interface that shows both parties' evidence, history, and account health side by side."
      },
    ],
  },
  ...
  "luxury-specialty": {
    ...
    intro: "Luxury retail contact centers are anti-volume. The goal is not efficiency: it's relationship depth. [[retail.ex.lux-basket]] The CX challenge is delivering concierge-grade service at scale without losing the intimacy that defines luxury.",
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.luxury-specialty.aht]]", note: "Long by design where the service is built around the relationship" },
      { metric: "FCR", avg: "[[retail.sv.luxury-specialty.fcr]]", note: "Driven by advisor product knowledge and authority to act" },
      { metric: "CSAT", avg: "[[retail.sv.luxury-specialty.csat]]", note: "Moves with how closely the service matches the brand promise" },
      { metric: "Containment", avg: "[[retail.sv.luxury-specialty.containment]]", note: "Kept low on purpose where clients expect a person" },
    ],
    layers: [
      { layer: 7, name: "Analytics & Governance", capabilities: ["Client satisfaction tied to purchase frequency and average order value","Client advisor performance: revenue attribution and relationship depth","Product expertise scoring: agents tested on brand knowledge","VIP client risk monitoring: engagement decline signals attrition","Brand voice consistency scoring across all interactions"], vendors: "NICE, Qualtrics, Salesforce", risk: "Service quality declining as brand scales without detection",
        ...
        pitfall: "Luxury brands apply standard contact center metrics (AHT targets, calls per hour) to concierge interactions and wonder why service quality drops. In luxury, [[retail.ex.lux-call]] Measure revenue per interaction and client retention rate, not handle time."
      },
      { layer: 6, name: "Routing & Orchestration", capabilities: ["Named client advisor routing with relationship continuity","Purchase history-aware routing matching clients to product category experts","VIP and top-client priority routing with zero-wait SLA","After-purchase care routing to repair, alteration, and care specialists","Event and appointment routing for in-store experiences and trunk shows"], vendors: "Genesys, NICE CXone, Salesforce", risk: "A top client reaching a general agent who doesn't know their purchase history",
        stack: [{ name: "Genesys Cloud", role: "CCaaS", why: "Named-advisor routing with full client context. When a top client calls, they reach their dedicated advisor who knows their purchase history, style preferences, and upcoming events.", href: "/vendors/genesys" }],
        pitfall: "Named-advisor routing only works if advisors have manageable client books. If each advisor carries several hundred clients, the personalization is thin. Named-advisor service needs smaller books, around [[retail.lux.book]] per advisor. This is a staffing model decision, not a technology decision, but the technology must support the model."
      },
      ...
      { layer: 4, name: "Reasoning & Planning", capabilities: ["Product availability and reservation bots for limited editions","Order and delivery status for high-value purchases","Repair and alteration status tracking","Personal shopping appointment scheduling","Basic brand and product information"], vendors: "Limited use: luxury clients prefer humans", risk: "Bot interaction degrading the luxury experience",
        stack: [{ name: "Minimal AI recommended", role: "None", why: "Luxury clients chose the brand for human craftsmanship and personal service. A chatbot greeting a top client with 'How can I help you today?' undercuts the brand promise. Use AI behind the scenes (agent assist, recommendation engines), not customer-facing." }],
        pitfall: "The luxury containment target should be close to none. Every client interaction is a revenue and relationship opportunity. [[retail.ex.lux-bot]] AI in luxury belongs in the agent's ear (real-time product knowledge, client preferences, inventory lookup), not in the client's face."
      },
      { layer: 3, name: "Policy & Guardrails", capabilities: ["Brand voice and communication standards enforcement","Client data privacy and confidentiality controls","PCI compliance for high-value transactions","Anti-counterfeiting and grey market monitoring","International trade compliance for cross-border luxury purchases"], vendors: "Custom brand standards, legal compliance", risk: "Brand voice inconsistency across client advisors",
        stack: [{ name: "Brand-specific training", role: "Standards", why: "Luxury compliance is brand-specific. Each house sets its own communication standards. The 'guardrails' in luxury are brand guidelines, not generic compliance tools. Invest in advisor training and certification over technology." }],
      ...
      { layer: 2, name: "Workflow Execution", capabilities: ["Made-to-order and customization workflow with client specification capture","Repair and restoration workflow with item tracking and progress updates","Personal shopping workflow with style profile and wish list management","VIP event management workflow with RSVP, preparation, and follow-up","Gift registry and corporate gifting workflow with personalization"], vendors: "Salesforce, custom CRM, Cegid", risk: "High-touch workflows automated into generic processes",
        stack: [{ name: "Salesforce / Custom CRM", role: "Client Management", why: "Luxury CRM must track preferences, sizing, past purchases, gifting history, and personal milestones (birthdays, anniversaries). Many luxury brands build custom CRM on Salesforce with heavily customized data models.", href: "/vendors/ccaas" }],
        pitfall: "Luxury workflows cannot be standardized the way mass retail workflows can. [[retail.ex.lux-repair]]: different SLAs, different communication cadence, different escalation paths. Build service-tier workflows, not one-size-fits-all workflows."
      },
      { layer: 1, name: "Data Access", capabilities: ["Client profile with complete purchase history across boutiques and online","Product catalog with specifications, availability, and waitlist status","Inventory across boutiques, warehouses, and partner locations","Client preferences, sizing, style notes, and wish lists","Event and appointment calendar integration"], vendors: "Salesforce, Cegid, custom systems", risk: "Client history in the advisor's head instead of in the system",
        ...
        pitfall: "The biggest data risk in luxury retail: client knowledge lives in the advisor's personal notebook, not in the CRM. When an advisor leaves, the brand can lose years of client relationship knowledge, sizing preferences, gift history, personal milestones, and style notes. Mandate CRM documentation as a performance requirement, not an optional task. The advisor's notebook belongs to the brand."
      },
    ],
  },
  ...
  "grocery-delivery": {
    ...
    intro: "Grocery and delivery contact centers operate in real-time. A customer who discovers a substitution they don't want, a late delivery, or a missing item needs it fixed while the order is still fresh. The CX challenge is speed and accuracy under pressure, with thin margins that make every unnecessary support interaction a direct hit to profitability.",
    kpis: [
      { metric: "AHT", avg: "[[retail.sv.grocery-delivery.aht]]", note: "Driven by concrete issues: wrong item, missing item, late delivery" },
      { metric: "FCR", avg: "[[retail.sv.grocery-delivery.fcr]]", note: "Many issues close with a refund or credit" },
      { metric: "CSAT", avg: "[[retail.sv.grocery-delivery.csat]]", note: "Moves with delivery delays and substitutions, whatever the support quality" },
      { metric: "Containment", avg: "[[retail.sv.grocery-delivery.containment]]", note: "Refunds, credits and status checks can be automated with fraud limits" },
    ],
    layers: [
      ...
      { layer: 5, name: "Conversation Management", capabilities: ["In-app messaging during active order with real-time shopper communication","Delivery status notifications with live driver tracking","Post-delivery feedback and issue reporting with photo evidence","Substitution approval/rejection flow during shopping","Proactive delay notifications with updated delivery windows"], vendors: "Custom in-app, Twilio, Zendesk", risk: "Customer discovering substitutions only at delivery with no recourse",
        ...
        pitfall: "Proactive substitution approval is among the most useful CX features in grocery delivery. When the shopper can't find organic eggs and selects conventional eggs as a substitute, the customer should be notified in real-time and given the option to approve, reject, or suggest an alternative. Surprise substitutions turn into refund requests and support contacts; measure the difference on your own orders."
      },
      ...
      { layer: 3, name: "Policy & Guardrails", capabilities: ["Refund and credit policy automation with per-customer limits","Food safety communication protocols for recalls and contamination","Age verification for alcohol delivery","Tipping and rating fairness policies for gig workers","Data privacy for delivery address and household information"], vendors: "Custom policies, age verification APIs", risk: "Unlimited automated refunds enabling systematic fraud",
        stack: [{ name: "Custom fraud controls", role: "Refund Guardrails", why: "Grocery platforms must build custom refund abuse detection because off-the-shelf fraud tools aren't designed for low-value produce claims. Per-customer refund velocity, photo evidence requirements above certain thresholds, and account review triggers.", href: "/vendors" }],
      ...
      { layer: 2, name: "Workflow Execution", capabilities: ["Real-time order modification workflow during active shopping","Refund and credit issuance workflow with root cause tagging","Substitution management workflow with preference learning","Delivery rescheduling workflow with window availability check","Recurring order workflow with pause, skip, and modification"], vendors: "Custom platforms, Shopify, Instacart Platform", risk: "Order modification requests not reaching shoppers in time",
        stack: [{ name: "Custom / Instacart Platform", role: "Grocery Operations", why: "Grocery delivery workflows are distinct enough that many platforms build their own. Instacart's white-label platform serves retailers who want the technology without building it.", href: "/vendors" }],
        pitfall: "A common grocery workflow failure: a customer messages 'please remove the milk' but the message reaches the support agent, not the shopper. The shopper has already picked and paid for the milk. Now the customer has milk they don't want and a support ticket to process. Real-time order modification must flow directly to the active shopper session."
      },
      { layer: 1, name: "Data Access", capabilities: ["Order management system with real-time order status and shopper assignment","Inventory management with store-level product availability","Delivery logistics with driver location and ETA","Customer order history with substitution preferences and refund history","Product catalog with pricing, descriptions, and allergen information"], vendors: "Custom OMS, Shopify, Instacart, DoorDash Drive", risk: "Inventory data not reflecting actual store shelf availability",
        ...
        pitfall: "Grocery inventory data is often inaccurate. A store's system says 'in stock' but the shelf is empty. The shopper can't find the item and makes a substitution the customer doesn't want. The entire substitution problem is an inventory accuracy problem. Real-time shelf-level inventory (camera-based, RFID) attacks the substitution problem at its source, and fewer substitutions mean fewer support contacts."
      },
    ],
  },
};
